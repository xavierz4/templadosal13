...
<div
  class="relative w-full aspect-square bg-[#050505] rounded-xl border border-white/10 shadow-inner overflow-hidden flex items-center justify-center group pointer-events-none"
>
  ...
  >
    ...
    <path
      d="M {PADDING / 2} {CANVAS_SIZE - PADDING} L {CANVAS_SIZE - PADDING / 2} {CANVAS_SIZE -
        PADDING}"
      stroke="#D4AF37"
      stroke-width="2"
    ...
    <g class="transform-origin-bottom">
      ...
      {:else if productType === 'fachada_monumental'}
        ...
        <rect
          x={rectX - 4}
          y={rectY - 4}
          width={$animatedWidth + 8}
          height={$animatedHeight + 8}
          fill="none"
          stroke="#555"
          stroke-width="6"
        />
      {:else if productType === 'puerta_pivotante'}
        ...
        <!-- Tirador / Manija -->
        <rect
          x={rectX + $animatedWidth - 15}
          y={rectY + $animatedHeight / 2 - 30}
          width="4"
          height="60"
          fill="#D4AF37"
          rx="2"
    ...
    <g class="font-mono text-[10px]" fill="#00F0FF">
      ...
      {#if width > 0}
        ...
        />
        <line
          x1={rectX + $animatedWidth}
          y1={rectY - 20}
          x2={rectX + $animatedWidth}
          y2={rectY - 10}
          stroke="#00F0FF"
          stroke-width="1"
        />
      ...
      {#if height > 0}
        <path
          d="M {rectX + $animatedWidth + 15} {rectY} L {rectX + $animatedWidth + 15} {rectY +
            $animatedHeight}"
          stroke="#00F0FF"
          stroke-width="1"
          class="opacity-50"
        ...
        <!-- Ticks -->
        <line
          x1={rectX + $animatedWidth + 10}
          y1={rectY}
          x2={rectX + $animatedWidth + 20}
          y2={rectY}
          stroke="#00F0FF"
          stroke-width="1"
        />
        <line
          x1={rectX + $animatedWidth + 10}
          y1={rectY + $animatedHeight}
          x2={rectX + $animatedWidth + 20}
          y2={rectY + $animatedHeight}
          stroke="#00F0FF"
          stroke-width="1"
        />
  ...
  <div class="absolute top-4 left-4 flex gap-2 items-center z-20">
    {#if hasError}
      ...
      <span
        class="text-xs font-mono text-[#FF3B30] font-bold uppercase tracking-widest bg-black/60 px-2 py-0.5 rounded border border-[#FF3B30]/30 backdrop-blur-sm"
        >NSR-10 FAILED</span
      >
    {:else if hasWarning}
      <div class="w-2 h-2 rounded-full bg-[#FF9500] shadow-[0_0_10px_#FF9500]"></div>
      <span
        class="text-xs font-mono text-[#FF9500] uppercase tracking-widest bg-black/60 px-2 py-0.5 rounded border border-[#FF9500]/30 backdrop-blur-sm"
        >NSR-10 WARNING</span
      >
    {:else}
      <div class="w-2 h-2 rounded-full bg-al13-cyan animate-pulse shadow-[0_0_10px_#00F0FF]"></div>
      <span
        class="text-xs font-mono text-al13-cyan uppercase tracking-widest bg-black/40 px-2 py-0.5 rounded border border-al13-cyan/20 backdrop-blur-sm"
        >Live Geometry</span
      >
    {/if}
  </div>
