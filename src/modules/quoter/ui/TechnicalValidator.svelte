<script lang="ts">
  ...
  let livePhysics = $derived(
    validateStructuralFeasibility({
  ...
  const steps = [
    {
      ...
      desc: 'Elige el sistema de vidrio y aluminio a cotizar.',
    },
...
<div class="w-full max-w-4xl mx-auto py-12 px-6">
  ...
  <div class="mb-12 text-center">
    ...
    <p class="text-zinc-400 font-light">Cálculo de Viabilidad Geométrica AL13</p>
  </div>

  <!-- Progress Bar B2B -->
  <div
    class="flex items-center justify-between mb-12 relative before:absolute before:top-1/2 before:-translate-y-1/2 before:h-[1px] before:w-full before:bg-white/10 before:-z-10"
  >
    {#each steps as step, i (i)}
      <div class="flex flex-col items-center gap-3 relative">
        ...
        <span
          class="text-xs font-semibold tracking-wider uppercase {currentStep >= i + 1
            ? 'text-white'
            : 'text-zinc-500'} absolute -bottom-6 w-32 text-center"
        >
          {step.title}
        </span>
      </div>
  ...
  <!-- Contenedor Principal (Glassmorphism) — Task 9.3.6: Materialización cristal templado -->
  <ScrollReveal animation="scale" duration={900} class="quoter-materialize">
    <div
      class="relative bg-white/5 backdrop-blur-[16px] border border-white/10 rounded-2xl p-8 md:p-12 shadow-[0_8px_32px_rgba(0,0,0,0.5)] min-h-[400px] quoter-glass-glow"
    >
        ...
        <div in:fade={{ duration: 400, delay: 100 }} out:fade={{ duration: 200 }} class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {#each products as product (product.id)}
              ...
              >
                <span class="text-3xl mr-4 grayscale group-hover:grayscale-0 transition-all"
                  >{product.icon}</span
                >
                <span
                  class="text-lg font-semibold text-zinc-200 group-hover:text-white transition-colors"
                  >{product.label}</span
                >
              </button>
            {/each}
        ...
        <div in:fade={{ duration: 400, delay: 100 }} out:fade={{ duration: 200 }} class="space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            ...
            <div class="flex flex-col gap-6">
              <!-- Input Ancho -->
              <div class="w-full">
                <label class="block text-sm font-semibold text-zinc-300 mb-2" for="width-input"
                  >Ancho del Vano (mm)</label
                >
                <div class="relative">
                  <input
                    id="width-input"
                  ...
                  />
                  <span
                    class="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 font-mono text-sm"
                    >mm</span
                  >
                </div>
              </div>
              ...
              <!-- Input Alto -->
              <div class="w-full">
                <label class="block text-sm font-semibold text-zinc-300 mb-2" for="height-input"
                  >Alto Libre (mm)</label
                >
                <div class="relative">
                  <input
          ...
          <div class="mt-8 flex gap-4 justify-center">
            <button
              onclick={prevStep}
              class="px-6 py-2 rounded border border-white/20 text-zinc-400 hover:text-white transition"
              >Atrás</button
            >
            <button
              onclick={nextStep}
        ...
        <div in:fade={{ duration: 400, delay: 100 }} out:fade={{ duration: 200 }} class="space-y-8">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
            {#each ['Incoloro Extra-Claro', 'Gris Humo', 'Bronce Espejo', 'Esmerilado Privacy'] as color (color)}
              ...
              >
                <div
                  class="w-12 h-12 rounded-full mb-3 border border-white/10 shadow-inner
      ...
      {#if currentStep === 4}
        ...
        >
          {#if physicsFeedback && !physicsFeedback.isValid}
            ...
            <div
              class="text-center w-full max-w-lg mb-8 p-6 bg-red-500/10 border border-red-500/50 rounded-xl"
            >
              <div
                class="w-16 h-16 mx-auto bg-red-500/20 text-red-500 rounded-full flex items-center justify-center mb-4 text-3xl"
              >
                ⚠️
              </div>
              ...
              <p class="text-sm font-mono text-zinc-400">
                Dimensión ingresada: {quoteData.width}x{quoteData.height}mm excede la matriz
                productiva (Jumbo).
              </p>
            </div>
            <div class="mt-8 flex gap-4">
              <button
                ...
                class="px-6 py-2 rounded border border-white/20 text-zinc-400 hover:text-white transition"
                >Ajustar Medidas (Volver)</button
              >
            </div>
          {:else if physicsFeedback && physicsFeedback.isValid}
            <!-- ESTADO ACEPTADO CONDICIONADO -->
            <div
              class="w-full max-w-2xl bg-black/40 border border-white/10 rounded-xl overflow-hidden shadow-2xl"
            >
              <!-- Cabecera Diagnóstico -->
              <div
                class="bg-gradient-to-r from-al13-cyan/20 to-transparent p-6 border-b border-al13-cyan/30 flex justify-between items-start"
              >
                <div>
                ...
                <div class="text-right">
                  <span class="block text-xs uppercase text-zinc-500 tracking-wider"
                    >Espesor Exigido</span
                  >
                  <span class="text-3xl font-heading font-bold text-white"
                    >{physicsFeedback.recommendedThickness}<span class="text-lg text-al13-cyan"
                      >mm</span
                    ></span
                  >
                </div>
              ...
              <div class="p-6 space-y-6">
                <div class="grid grid-cols-2 gap-4 text-sm">
                  <div class="bg-white/5 p-4 rounded-lg border border-white/5">
                    ...
                    <span class="text-white font-mono"
                      >{quoteData.width} x {quoteData.height} mm</span
                    >
                  </div>
                  <div class="bg-white/5 p-4 rounded-lg border border-white/5">
                ...
                {#if physicsFeedback.warnings.length > 0}
                  <div class="bg-yellow-500/10 border-l-4 border-yellow-500 p-4 rounded-r-lg">
                    <span class="text-yellow-500 font-bold mb-1 flex items-center gap-2"
                      ><span>🛡️</span> Carga Estructural Detectada
                    </span>
                ...
                <div
                  class="mt-8 p-6 bg-gradient-to-br from-al13-cyan/10 to-[#111] rounded-xl border border-al13-cyan/30 text-center"
                >
                  {#if submitSuccess}
                    <div in:fade class="py-4">
                      <div
                        class="w-16 h-16 mx-auto bg-green-500/20 text-green-500 rounded-full flex items-center justify-center mb-4 text-3xl"
                      >
                        ✓
                      </div>
                      <h4 class="text-white font-bold text-xl mb-2">Solicitud Recibida</h4>
                      <p class="text-zinc-400 text-sm">
                        Nuestro equipo de ingeniería revisará los cálculos y contactará a {quoteData.contactName ||
                          'su empresa'} en la brevedad.
                      </p>
                    </div>
                  {:else}
                    ...
                    <form
                      class="space-y-4 text-left max-w-sm mx-auto mb-6"
                      onsubmit={(e) => {
                        e.preventDefault();
                        submitLead();
                      }}
                    >
                      <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1" for="name"
                          >Nombre Completo *</label
                        >
                        <input
                          bind:value={quoteData.contactName}
                          required
                          type="text"
                          id="name"
                          class="w-full bg-[#0A0A0A]/50 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-al13-cyan focus:ring-1 focus:ring-al13-cyan"
                          placeholder="Ej. Arquitecto Juan Pérez"
                        />
                      </div>
                      <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1" for="company"
                          >Empresa / Firma</label
                        >
                        <input
                          bind:value={quoteData.companyName}
                          type="text"
                          id="company"
                          class="w-full bg-[#0A0A0A]/50 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-al13-cyan focus:ring-1 focus:ring-al13-cyan"
                      ...
                      <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1" for="phone"
                          >Teléfono de Contacto *</label
                        >
                        <input
                          bind:value={quoteData.phone}
                          required
                          type="tel"
                          id="phone"
                          class="w-full bg-[#0A0A0A]/50 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-al13-cyan focus:ring-1 focus:ring-al13-cyan"
                      ...
                      <button
                        type="submit"
                        disabled={isSubmitting || !quoteData.contactName || !quoteData.phone}
                        class="w-full mt-4 py-3 rounded bg-al13-cyan text-black font-bold hover:bg-al13-emerald transition shadow-[0_0_15px_rgba(212,175,55,0.4)] disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
                      >
                        {#if isSubmitting}
                          <span
                            class="animate-spin h-5 w-5 mr-2 border-2 border-black border-t-transparent rounded-full"
                          ></span> Procesando...
                        {:else}
            ...
            <div class="mt-6 flex justify-center">
              <button
                ...
                class="text-xs text-zinc-500 hover:text-white underline transition"
                >Reiniciar Motor Físico</button
              >
            </div>
          {/if}
        </div>
...
<style>
  /* Task 9.3.6: Post-entry glow — sutil halo de cristal tras materialización */
  :global(.quoter-materialize.is-visible) .quoter-glass-glow,
  :global(.quoter-materialize.scroll-reveal--native) .quoter-glass-glow {
    animation: quoter-glow-appear 1.2s ease-out 0.9s forwards;
  }

  @keyframes quoter-glow-appear {
    from {
  ...
  /* Accesibilidad: sin glow animation */
